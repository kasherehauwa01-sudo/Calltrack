...
```bash
php scripts/reset_web_user_pin.php user@example.com
```

Команда сохраняет новый `password_hash`, активирует пользователя и очищает накопленные неудачные попытки. Для входа используется именно PIN web-пользователя, а не пароль его почтового ящика.
