...
try {
    ...
    if ($action==='login') {
        $data=readJsonBody(); $login=strtolower(trim((string)($data['login']??''))); $pin=(string)($data['pin']??'');
        if($login===''||!preg_match('/^\d{4,12}$/',$pin))sendJson(['status'=>'error','message'=>'Неверный логин или PIN-код'],401);
        ...
        if (!$row || !(int)$row['is_active'] || !password_verify($pin,(string)$row['pin_hash'])) {
            ...
            sendJson(['status'=>'error','message'=>'Неверный логин или PIN-код'],401);
        }
