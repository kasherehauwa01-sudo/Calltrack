...
try{
    ...
    if($action==='login'){
        ...
        $stmt=$pdo->prepare('SELECT id,display_name,login,pin_hash,role,is_active FROM web_users WHERE login=:login LIMIT 1');$stmt->execute([':login'=>$login]);$user=$stmt->fetch(PDO::FETCH_ASSOC);
        ...
        unset($user['pin_hash'],$user['is_active']);$user['user_phone']=webUserPhone((int)$user['id']);sendJson(['status'=>'success','data'=>['token'=>$raw,'user'=>$user]]);
    }
    ...
    $user['user_phone']=webUserPhone((int)$user['id']);sendJson(['status'=>'success','data'=>['user'=>$user]]);
