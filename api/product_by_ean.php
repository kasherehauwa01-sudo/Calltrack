...
if (strlen($ean13) !== 13) {
    sendJson(['status'=>'error', 'message'=>'Передайте штрихкод EAN-13'], 400);
}

$sum = 0;
for ($index = 0; $index < 12; $index++) {
    $sum += (int)$ean13[$index] * ($index % 2 === 0 ? 1 : 3);
}
if ((10 - $sum % 10) % 10 !== (int)$ean13[12]) {
    sendJson(['status'=>'error', 'message'=>'Некорректная контрольная сумма EAN-13'], 400);
}
...
if (!is_array($products)) {
    sendJson(['status'=>'error', 'message'=>'Локальный кэш товаров повреждён'], 500);
}
...
foreach ($products as $product) {
    ...
    sendJson(['status'=>'success', 'data'=>[
        'ean13'=>$ean13,
        'name'=>$name !== '' ? $name : $ean13,
        'fields'=>is_array($product['fields'] ?? null) ? $product['fields'] : $product,
    ]]);
}
