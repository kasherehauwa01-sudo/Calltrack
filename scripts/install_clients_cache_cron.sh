...
0 4 * * * CALLTRACK_PROJECT_DIR=$(printf '%q' "$PROJECT_DIR") /usr/bin/env bash $(printf '%q' "$PROJECT_DIR/scripts/refresh_clients_cache_cron.sh") delta >/dev/null 2>&1
0 3 * * 0 CALLTRACK_PROJECT_DIR=$(printf '%q' "$PROJECT_DIR") /usr/bin/env bash $(printf '%q' "$PROJECT_DIR/scripts/refresh_clients_cache_cron.sh") full >/dev/null 2>&1
$MARKER END
EOF
...
echo "Cron Clients установлен: delta ежедневно 04:00, full воскресенье 03:00 Europe/Moscow"
