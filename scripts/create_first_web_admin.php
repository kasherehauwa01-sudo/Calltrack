...
if(PHP_SAPI!=='cli')exit(1);$login=trim((string)($argv[1]??''));$name=trim((string)($argv[2]??''));
fwrite(STDOUT,'PIN: ');if(function_exists('shell_exec'))@shell_exec('stty -echo');$pin=trim((string)fgets(STDIN));if(function_exists('shell_exec'))@shell_exec('stty echo');fwrite(STDOUT,"\n");if($login===''||$name===''||$pin==='')throw new RuntimeException('Использование: php scripts/create_first_web_admin.php LOGIN "ФИО"');
