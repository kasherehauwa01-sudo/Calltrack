...
константой `CLIENTS_API_URL` в `api/config.local.php`. В колонке **Телефоны** может находиться
...
В админ-панели на вкладке **Общие настройки** доступен ручной **Тест API с clients**.
Запрос из браузера передаёт номер в `api/test_clients.php`; endpoint обращается напрямую
к `CLIENTS_API_URL`, нормализует номер до `+7XXXXXXXXXX` и возвращает найденные наименования или причину отсутствия результата.
