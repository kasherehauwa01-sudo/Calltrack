...
### Проверенная версия скрипта Calltrack (исправления)

Ниже версия с исправлениями, которые критичны для приложения:
- `doGet` поддерживает фильтр и по `phone`, и по `user_phone` (приложение использует `phone` при загрузке истории);
- в `doPost`/`doGet` есть защита от отсутствующих колонок;
- возвращается JSON даже при ошибках (приложение не падает на парсинге).

```javascript
var CALLTRACK_SPREADSHEET_ID = "1ZNvJK0s1KkoOY9M-iIvNBAX07YBBcSFTaht1smCQDLc";
var CALLS_SHEET_NAME = "Calls";
...
function getCallsSheet() {
  var ss = SpreadsheetApp.openById(CALLTRACK_SPREADSHEET_ID);
  var sheet = ss.getSheetByName(CALLS_SHEET_NAME);
  if (!sheet) throw new Error("Лист Calls не найден");
  return sheet;
}

function ensureHeaders(sheet) {
  var required = ["Дата","Время","Номер телефона","Тип звонка","Длительность","Менеджер","Комментарий","Тег","Напоминание","Текст напоминания","Клиент","ID","Номер телефона пользователя"];
  if (sheet.getLastRow() === 0) sheet.appendRow(required);
  return sheet.getRange(1, 1, 1, required.length).getValues()[0];
}

function getHeaderMap(header) {
  var map = {};
  for (var i = 0; i < header.length; i++) map[String(header[i] || "").trim()] = i;
  return map;
}

function doPost(e) {
  try {
    if (!e || !e.postData) return jsonResponse({status:"error", message:"NO_POST_DATA"});

    var body = e.postData.contents || "";
    var data = JSON.parse(body || "{}");
    if (!data.phone) return jsonResponse({status:"error", message:"PHONE_REQUIRED"});
    ...
    if (!updated) sheet.appendRow(row);
    return jsonResponse({status:"success", mode: updated ? "updated" : "inserted"});
  } catch (err) {
    return jsonResponse({status:"error", message:String(err)});
  }
}
...
function doGet(e) {
  try {
    ...
    var filterUserPhone = normalizePhone((e && e.parameter && e.parameter.user_phone) || "");
    var filterPhone = normalizePhone((e && e.parameter && e.parameter.phone) || "");

    var result = [];
    for (var i = data.length - 1; i >= 1; i--) {
      var row = data[i];
      var rowPhone = normalizePhone(row[map["Номер телефона"]]);
      var rowUserPhone = normalizePhone(row[map["Номер телефона пользователя"]]);

      if (filterPhone && rowPhone !== filterPhone) continue;
      if (filterUserPhone && rowUserPhone !== filterUserPhone) continue;
