...
Для публикационного APK выполните обычную release-сборку:
...
Перед первой release-сборкой скопируйте `keystore.properties.example` в
`keystore.properties` и укажите **существующий** JKS-ключ, которым подписана уже
установленная версия CallTrack. Сам файл настроек и JKS-файлы исключены из Git.
Без этих настроек release-сборка завершается ошибкой, чтобы на сервер случайно
не попал APK с другой подписью. Новый ключ не сможет обновить уже установленное
приложение — необходимо использовать исходный ключ или один раз переустановить
приложение с потерей совместимости обновления со старой подписью.

```properties
storeFile=/absolute/path/to/calltrack-release.jks
storePassword=...
keyAlias=calltrack
keyPassword=...
```

`versionCode` автоматически вычисляется при конфигурации сборки по формуле
`System.currentTimeMillis() / 60000L`. Поэтому release APK, собранные в разные
минуты, получают возрастающие номера без файла-счётчика и ручного изменения
Gradle-конфигурации. `versionName` остаётся равным `1.0.15` и не связан с
`versionCode`. Две сборки в пределах одной минуты могут получить одинаковый код;
для повторной публикационной сборки дождитесь следующей минуты.

Готовый APK находится в `app/build/outputs/apk/release/`. Его имя содержит
фактические `versionName` и `versionCode`.
Release APK должен подписываться тем же существующим ключом, что и установленное
приложение; механизм версионирования подпись не меняет.
Release-сборка явно содержит подписи v1 и v2: это позволяет устанавливать APK
не только на актуальные версии Android, но и через системный установщик старых
прошивок Samsung (включая Android 10). Перед публикацией рекомендуется проверить
готовый файл командой `apksigner verify --verbose --print-certs <apk>`: строки
`Verified using v1 scheme` и `Verified using v2 scheme` должны иметь значение
`true`.
