...
Для публикационного APK используйте только специальную задачу:

```bash
./gradlew :app:publishReleaseApk
# если Gradle Wrapper отсутствует:
gradle :app:publishReleaseApk
```

Задача читает последний успешно использованный номер из `version.properties`, под
межпроцессной блокировкой вычисляет следующий последовательный номер, собирает
`release` APK и проверяет его реальные `versionName`/`versionCode` через `aapt`.
Только после успешной сборки и проверки новый номер атомарно записывается обратно.
При `Gradle Sync`, `clean`, тестах, обычном Run, `assembleDebug` и даже прямом
`assembleRelease` счётчик не изменяется.

При `VERSION_CODE=15` первая публикационная сборка создаст
`app/build/outputs/apk/release/calltrack_v1.0.16_16.apk` и сохранит код 16;
следующая — `calltrack_v1.0.17_17.apk` и код 17. Параметр
`VERSION_NAME_PREFIX=1.0` отвечает за первые две части отображаемой версии.
Release APK должен подписываться тем же существующим ключом, что и установленное
приложение; механизм подписи задача не меняет.
