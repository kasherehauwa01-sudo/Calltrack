...
$pdo->exec('CREATE TABLE web_users (id INTEGER PRIMARY KEY, login TEXT UNIQUE, pin_hash TEXT, display_name TEXT, role TEXT, is_active INTEGER)');
$oldHash = password_hash('1111', PASSWORD_DEFAULT);
$pdo->prepare('INSERT INTO web_users VALUES (1, :login, :hash, :name, :role, 0)')->execute([':login'=>'admin', ':hash'=>$oldHash, ':name'=>'Администратор', ':role'=>'admin']);
...
if ($row['login'] !== 'admin' || $row['display_name'] !== 'Администратор' || $row['role'] !== 'admin' || (int)$row['is_active'] !== 0) throw new RuntimeException('Сброс PIN изменил другие поля пользователя');
